...
void B2GEdmExtraVarProducer::calculate_variables(edm::Event const& iEvent, edm::EventSetup const& iSetup) {
  ...
  if (!isData_) {
    ...
    if (single_float_["MC_t_pt"]!=-9999 && single_float_["MC_tbar_pt"]!=-9999) {
      ...
      if (lepID!=0 && single_float_["MC_hadW_pt"]!=-9999.) {
        if ((first_b_ID*lepID)<0) {
          //std::cout << "bs are leptonic, hadronic\n"; //DEBUG
          ...
          //std::cout << "bs are hadronic, leptonic\n"; //DEBUG
    ...
    for (size_t i=0; i<ngen; ++i) {
      ...
      if (h_floats_["gen_Mom0ID"]->at(i)==2212||h_floats_["gen_Mom1ID"]->at(i)==2212||abs(h_floats_["gen_ID"]->at(i))==5||abs(h_floats_["gen_ID"]->at(i))==6||
